defmodule ForkWithFlags.GateTest do
  ...
  describe "new()" do
    ...
    test "new(:percentage_of_actors, ratio_f) retrurns a PercentageOfActors gate" do
      assert %Gate{type: :percentage_of_actors, for: 0.001, enabled: true} = Gate.new(:percentage_of_actors, 0.001)
      assert %Gate{type: :percentage_of_actors, for: 0.1, enabled: true} = Gate.new(:percentage_of_actors, 0.1)
      assert %Gate{type: :percentage_of_actors, for: 0.59, enabled: true} = Gate.new(:percentage_of_actors, 0.59)
      assert %Gate{type: :percentage_of_actors, for: 0.999, enabled: true} = Gate.new(:percentage_of_actors, 0.999)
    end
  ...
  describe "enabled?(gate, for: actor, flag_name: atom), for PercentageOfActors gates" do
    setup do
      ...
      gandalf = %TestUser{id: 42, email: "gandalf@travels.com" } # with coconut: 0.7024383544921875
      magneto = %TestUser{id: 2, email: "magneto@mutants.com" } # with coconut: 0.4715118408203125
    ...
    test "without the [flag_name: atom] option it raises an exception", %{gate: gate, gandalf: gandalf} do
      assert_raise KeyError, fn() ->
